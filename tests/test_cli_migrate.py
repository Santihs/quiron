...
def test_migrate_new_vault_applies_directly(tmp_path, capsys):
    ...
    rc = main(["migrate", "--vault", str(vault_path), "--subject-expertise", "interview prep"])
    assert rc == 0
...
def test_migrate_existing_vault_defaults_to_dry_run(tmp_path, capsys):
    ...
    rc = main(
        ["migrate", "--vault", str(vault_path), "--subject-expertise", "v2", "--dry-run=false"]
    )
    assert rc == 0
    out = capsys.readouterr().out
    assert "dry run" not in out
    ...
    reviewer = (vault_path / ".claude" / "agents" / "quiz-reviewer.md").read_text(encoding="utf-8")
